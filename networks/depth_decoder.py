...
class DepthDecoder(nn.Module):
    ...
    def nestConv(self, conv, high_feature, low_features):  # 高层特征图vs底层特征图
        conv_0 = conv[0]
        conv_1 = conv[1]
        assert isinstance(low_features, list)  # 列表保存特征图数量111223
    ...
    def forward(self, input_features):
        ...
        for index in self.all_position:
            ...
            low_features = []
            for i in range(col):
                low_features.append(features["X_{}{}".format(row, i)])  # 构建低分辨率图像列表
            ...
            if index in self.attention_position:
                features["X_" + index] = self.convs["X_" + index + "_attention"](
                    ...
                    low_features)
            elif index in self.non_attention_position:  # 两个卷积操作 列表形式
