    ...
    if opt.ext_disp_to_eval is None:
        ...
        with torch.no_grad():
            for data in dataloader:
                ...
                # # 打印当前 batch 的 disp / depth 范围
                # disp_min = output[("disp", 0)].min().item()
                # disp_max = output[("disp", 0)].max().item()
                # depth_min = pred_depth.min().item()
                # depth_max = pred_depth.max().item()
                #
                # print(f"[disp] {disp_min:.4f} ~ {disp_max:.4f} | [depth] {depth_min:.4f} ~ {depth_max:.4f}")
